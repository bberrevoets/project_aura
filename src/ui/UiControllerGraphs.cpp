...
void UiController::ensure_pm05_graph_overlays() {
    ...
    auto ensure_label = [this](lv_obj_t *&label, lv_align_t align, lv_coord_t x_ofs, lv_coord_t y_ofs) {
        if (!label || !lv_obj_is_valid(label) || lv_obj_get_parent(label) != objects.chart_pm05_info) {
            label = lv_label_create(objects.chart_pm05_info);
            lv_obj_clear_flag(label, LV_OBJ_FLAG_CLICKABLE | LV_OBJ_FLAG_SCROLLABLE);
            lv_obj_set_style_text_font(label, &ui_font_jet_reg_14, LV_PART_MAIN | LV_STATE_DEFAULT);
            lv_obj_set_style_pad_left(label, 8, LV_PART_MAIN | LV_STATE_DEFAULT);
            lv_obj_set_style_pad_right(label, 8, LV_PART_MAIN | LV_STATE_DEFAULT);
    ...
    ensure_label(pm05_graph_label_min_, LV_ALIGN_BOTTOM_LEFT, 8, -6);
    ensure_label(pm05_graph_label_now_, LV_ALIGN_TOP_LEFT, 8, 6);
    ensure_label(pm05_graph_label_max_, LV_ALIGN_TOP_RIGHT, -8, 6);
}
...
void UiController::update_pm05_graph_overlays(bool has_values,
    ...
    const lv_color_t border = lv_obj_get_style_border_color(objects.chart_pm05_info, LV_PART_MAIN);
    const lv_color_t text = active_text_color();
    const lv_color_t badge_bg = lv_color_mix(border, chart_bg, LV_OPA_60);

    lv_obj_t *labels[] = {pm05_graph_label_min_, pm05_graph_label_now_, pm05_graph_label_max_};
    for (lv_obj_t *label : labels) {
        lv_obj_set_style_text_color(label, text, LV_PART_MAIN | LV_STATE_DEFAULT);
        lv_obj_set_style_bg_color(label, badge_bg, LV_PART_MAIN | LV_STATE_DEFAULT);
        lv_obj_set_style_border_color(label, border, LV_PART_MAIN | LV_STATE_DEFAULT);
...
void UiController::ensure_pm05_time_labels() {
    ...
    auto ensure_label = [this](lv_obj_t *&label) {
        if (!label || !lv_obj_is_valid(label) || lv_obj_get_parent(label) != objects.pm05_info_graph) {
            label = lv_label_create(objects.pm05_info_graph);
            lv_obj_clear_flag(label, LV_OBJ_FLAG_CLICKABLE | LV_OBJ_FLAG_SCROLLABLE);
            lv_obj_set_style_text_font(label, &ui_font_jet_reg_14, LV_PART_MAIN | LV_STATE_DEFAULT);
            lv_obj_set_style_bg_opa(label, LV_OPA_TRANSP, LV_PART_MAIN | LV_STATE_DEFAULT);
            lv_obj_set_style_border_width(label, 0, LV_PART_MAIN | LV_STATE_DEFAULT);
    ...
    for (uint8_t i = 0; i < kTempGraphTimeTickCount; ++i) {
        ensure_label(pm05_graph_time_labels_[i]);
    }

    const lv_color_t border = lv_obj_get_style_border_color(objects.chart_pm05_info, LV_PART_MAIN);
    const lv_color_t text = lv_color_mix(active_text_color(), border, LV_OPA_30);
    for (uint8_t i = 0; i < kTempGraphTimeTickCount; ++i) {
        lv_obj_t *label = pm05_graph_time_labels_[i];
        if (!label) {
            continue;
        }
        lv_obj_set_style_text_color(label, text, LV_PART_MAIN | LV_STATE_DEFAULT);
...
void UiController::update_pm05_time_labels() {
    if (!objects.chart_pm05_info || !objects.pm05_info_graph) {
        return;
    }

    ensure_pm05_time_labels();
    if (!pm05_graph_time_labels_[0]) {
        return;
    }
    ...
    if (points < 2) {
        for (uint8_t i = 0; i < kTempGraphTimeTickCount; ++i) {
            if (pm05_graph_time_labels_[i]) {
                safe_label_set_text(pm05_graph_time_labels_[i], "");
            }
        }
        return;
    }
    ...
    for (uint8_t i = 0; i < kTempGraphTimeTickCount; ++i) {
        ...
        if (absolute_time) {
            const time_t tick_epoch = end_epoch - static_cast<time_t>(offset_s);
            formatted = format_epoch_hhmm(tick_epoch, text, sizeof(text));
        }
        if (!formatted) {
            format_relative_time_label(offset_s, text, sizeof(text));
        }

        safe_label_set_text(label, text);
    }
...
void UiController::ensure_pm25_4_graph_overlays() {
    ...
    auto ensure_label = [this](lv_obj_t *&label, lv_align_t align, lv_coord_t x_ofs, lv_coord_t y_ofs) {
        if (!label || !lv_obj_is_valid(label) || lv_obj_get_parent(label) != objects.chart_pm25_4_graph) {
            label = lv_label_create(objects.chart_pm25_4_graph);
            lv_obj_clear_flag(label, LV_OBJ_FLAG_CLICKABLE | LV_OBJ_FLAG_SCROLLABLE);
            lv_obj_set_style_text_font(label, &ui_font_jet_reg_14, LV_PART_MAIN | LV_STATE_DEFAULT);
            lv_obj_set_style_pad_left(label, 8, LV_PART_MAIN | LV_STATE_DEFAULT);
            lv_obj_set_style_pad_right(label, 8, LV_PART_MAIN | LV_STATE_DEFAULT);
    ...
    ensure_label(pm25_4_graph_label_min_, LV_ALIGN_BOTTOM_LEFT, 8, -6);
    ensure_label(pm25_4_graph_label_now_, LV_ALIGN_TOP_LEFT, 8, 6);
    ensure_label(pm25_4_graph_label_max_, LV_ALIGN_TOP_RIGHT, -8, 6);
}
...
void UiController::update_pm25_4_graph_overlays(bool has_values,
    ...
    const lv_color_t border = lv_obj_get_style_border_color(objects.chart_pm25_4_graph, LV_PART_MAIN);
    const lv_color_t text = active_text_color();
    const lv_color_t badge_bg = lv_color_mix(border, chart_bg, LV_OPA_60);

    lv_obj_t *labels[] = {pm25_4_graph_label_min_, pm25_4_graph_label_now_, pm25_4_graph_label_max_};
    for (lv_obj_t *label : labels) {
        lv_obj_set_style_text_color(label, text, LV_PART_MAIN | LV_STATE_DEFAULT);
        lv_obj_set_style_bg_color(label, badge_bg, LV_PART_MAIN | LV_STATE_DEFAULT);
        lv_obj_set_style_border_color(label, border, LV_PART_MAIN | LV_STATE_DEFAULT);
...
void UiController::ensure_pm25_4_time_labels() {
    ...
    auto ensure_label = [this](lv_obj_t *&label) {
        if (!label || !lv_obj_is_valid(label) || lv_obj_get_parent(label) != objects.pm25_4_graph) {
            label = lv_label_create(objects.pm25_4_graph);
            lv_obj_clear_flag(label, LV_OBJ_FLAG_CLICKABLE | LV_OBJ_FLAG_SCROLLABLE);
            lv_obj_set_style_text_font(label, &ui_font_jet_reg_14, LV_PART_MAIN | LV_STATE_DEFAULT);
            lv_obj_set_style_bg_opa(label, LV_OPA_TRANSP, LV_PART_MAIN | LV_STATE_DEFAULT);
            lv_obj_set_style_border_width(label, 0, LV_PART_MAIN | LV_STATE_DEFAULT);
    ...
    for (uint8_t i = 0; i < kTempGraphTimeTickCount; ++i) {
        ensure_label(pm25_4_graph_time_labels_[i]);
    }

    const lv_color_t border = lv_obj_get_style_border_color(objects.chart_pm25_4_graph, LV_PART_MAIN);
    const lv_color_t text = lv_color_mix(active_text_color(), border, LV_OPA_30);
    for (uint8_t i = 0; i < kTempGraphTimeTickCount; ++i) {
        lv_obj_t *label = pm25_4_graph_time_labels_[i];
        if (!label) {
            continue;
        }
        lv_obj_set_style_text_color(label, text, LV_PART_MAIN | LV_STATE_DEFAULT);
...
void UiController::update_pm25_4_time_labels() {
    if (!objects.chart_pm25_4_graph || !objects.pm25_4_graph) {
        return;
    }

    ensure_pm25_4_time_labels();
    if (!pm25_4_graph_time_labels_[0]) {
        return;
    }
...
void UiController::ensure_pm1_10_graph_overlays() {
    ...
    auto ensure_label = [this](lv_obj_t *&label, lv_align_t align, lv_coord_t x_ofs, lv_coord_t y_ofs) {
        if (!label || !lv_obj_is_valid(label) || lv_obj_get_parent(label) != objects.chart_pm1_10_info) {
            label = lv_label_create(objects.chart_pm1_10_info);
            lv_obj_clear_flag(label, LV_OBJ_FLAG_CLICKABLE | LV_OBJ_FLAG_SCROLLABLE);
            lv_obj_set_style_text_font(label, &ui_font_jet_reg_14, LV_PART_MAIN | LV_STATE_DEFAULT);
            lv_obj_set_style_pad_left(label, 8, LV_PART_MAIN | LV_STATE_DEFAULT);
            lv_obj_set_style_pad_right(label, 8, LV_PART_MAIN | LV_STATE_DEFAULT);
    ...
    ensure_label(pm1_10_graph_label_min_, LV_ALIGN_BOTTOM_LEFT, 8, -6);
    ensure_label(pm1_10_graph_label_now_, LV_ALIGN_TOP_LEFT, 8, 6);
    ensure_label(pm1_10_graph_label_max_, LV_ALIGN_TOP_RIGHT, -8, 6);
}
...
void UiController::update_pm1_10_graph_overlays(bool has_values,
    ...
    const lv_color_t border = lv_obj_get_style_border_color(objects.chart_pm1_10_info, LV_PART_MAIN);
    const lv_color_t text = active_text_color();
    const lv_color_t badge_bg = lv_color_mix(border, chart_bg, LV_OPA_60);

    lv_obj_t *labels[] = {pm1_10_graph_label_min_, pm1_10_graph_label_now_, pm1_10_graph_label_max_};
    for (lv_obj_t *label : labels) {
        lv_obj_set_style_text_color(label, text, LV_PART_MAIN | LV_STATE_DEFAULT);
        lv_obj_set_style_bg_color(label, badge_bg, LV_PART_MAIN | LV_STATE_DEFAULT);
        lv_obj_set_style_border_color(label, border, LV_PART_MAIN | LV_STATE_DEFAULT);
...
void UiController::ensure_pm1_10_time_labels() {
    ...
    auto ensure_label = [this](lv_obj_t *&label) {
        if (!label || !lv_obj_is_valid(label) || lv_obj_get_parent(label) != objects.pm1_10_info_graph) {
            label = lv_label_create(objects.pm1_10_info_graph);
            lv_obj_clear_flag(label, LV_OBJ_FLAG_CLICKABLE | LV_OBJ_FLAG_SCROLLABLE);
            lv_obj_set_style_text_font(label, &ui_font_jet_reg_14, LV_PART_MAIN | LV_STATE_DEFAULT);
            lv_obj_set_style_bg_opa(label, LV_OPA_TRANSP, LV_PART_MAIN | LV_STATE_DEFAULT);
            lv_obj_set_style_border_width(label, 0, LV_PART_MAIN | LV_STATE_DEFAULT);
    ...
    for (uint8_t i = 0; i < kTempGraphTimeTickCount; ++i) {
        ensure_label(pm1_10_graph_time_labels_[i]);
    }

    const lv_color_t border = lv_obj_get_style_border_color(objects.chart_pm1_10_info, LV_PART_MAIN);
    const lv_color_t text = lv_color_mix(active_text_color(), border, LV_OPA_30);
    for (uint8_t i = 0; i < kTempGraphTimeTickCount; ++i) {
        lv_obj_t *label = pm1_10_graph_time_labels_[i];
        if (!label) {
            continue;
        }
        lv_obj_set_style_text_color(label, text, LV_PART_MAIN | LV_STATE_DEFAULT);
...
void UiController::update_pm1_10_time_labels() {
    if (!objects.chart_pm1_10_info || !objects.pm1_10_info_graph) {
        return;
    }

    ensure_pm1_10_time_labels();
    if (!pm1_10_graph_time_labels_[0]) {
        return;
    }
